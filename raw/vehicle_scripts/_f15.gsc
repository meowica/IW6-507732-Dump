...
{
	build_template( "f15", model, type, classname );
	...
	build_deathfx( "fx/explosions/large_vehicle_explosion", undefined, "explo_metal_rand", undefined, undefined, undefined, undefined, undefined, undefined, 0 );
	...
	build_light( classname, "wingtip_green"	 , "TAG_LEFT_WINGTIP"	, "fx/misc/aircraft_light_wingtip_green", "running", randomStartDelay );
	build_light( classname, "tail_green"	 , "TAG_LEFT_TAIL"		, "fx/misc/aircraft_light_wingtip_green", "running", randomStartDelay );
	build_light( classname, "wingtip_red"	 , "TAG_RIGHT_WINGTIP"	, "fx/misc/aircraft_light_wingtip_red"	 , "running", randomStartDelay );
	build_light( classname, "tail_red"		 , "TAG_RIGHT_TAIL"		, "fx/misc/aircraft_light_wingtip_red"	 , "running", randomStartDelay );
	build_light( classname, "white_blink"	 , "TAG_LIGHT_BELLY"	, "fx/misc/aircraft_light_white_blink"	 , "running", randomStartDelay );
	build_light( classname, "landing_light01", "TAG_LIGHT_LANDING01", "fx/misc/light_mig29_landing"		 , "landing", 0.0 );
...
playConTrail()
{
	...
	for ( ;; )
	{
		...
		playfxontag( contrail, self.tag2, "tag_origin" );
		ent_flag_waitopen( "contrails" );
		stopfxontag( contrail, self.tag1, "tag_origin" );
		stopfxontag( contrail, self.tag2, "tag_origin" );
	}
...
plane_bomb_node()
{
	//FX and soundalias will need to be added to your level if you use this.
	
	level._effect[ "plane_bomb_explosion1" ]			 = loadfx( "fx/explosions/bomb_explosion_ac130_small" );//100ton_bomb //fuel_storage
	...
	//aBomb_targets = getentarray( self.script_linkTo, "script_linkname" ); Sholmes: 3.2011 - this no longer works -  instead get_linked_ents()
...
plane_bomb_cluster()
{
	...
	for ( i = 0; i < repeat; i++ )
	{
		...
		if ( i%3 == 0 )
		{
			thread play_sound_in_space( "airstrike_explosion_close", traceHit );
...
/*QUAKED script_vehicle_f15_low (1 0 0) (-16 -16 -24) (16 16 32) USABLE SPAWNER
...
include,vehicle_f15_low
sound,vehicle_f15,vehicle_standard,all_sp


defaultmdl="vehicle_f15_low"
default:"vehicletype" "f15"
default:"script_team" "allies"
*/
