...
/*QUAKED script_model_pickup_sentry_smg (1 0 0) (-32 -16 0) (32 16 24) ORIENT_LOD NO_SHADOW  NO_STATIC_SHADOWS
defaultmdl="sentry_smg_animated"
default:"model" "sentry_smg_animated"
*/
...
setup_sentry_globals()
{
	if ( isSP() )
	{
		// These should be precached as hints, not strings
...
setup_sentry_smg()
{
	...
	array_thread( getentarray( "script_model_pickup_sentry_smg", "classname" ), ::sentry_pickup_init, "sentry_smg" );
}
...
sentry_smg_default_settings( type )
{
	level.sentry_settings[ type ].ammo 		 		 = 100;
	level.sentry_settings[ type ].use_laser 		 = true;
	level.sentry_settings[ type ].burst_shots_min 	 = 10;
	...
	level.sentry_settings[ type ].burst_pause_max 	 = 1.5;
	level.sentry_settings[ type ].fire_only_on_target= true;
	level.sentry_settings[ type ].model 			 = "weapon_sentry_smg";
	level.sentry_settings[ type ].destroyedModel 	 = "weapon_sentry_smg_destroyed";
	level.sentry_settings[ type ].pickupModel 		 = "weapon_sentry_smg_animated";
	level.sentry_settings[ type ].pickupModelObj 	 = "weapon_sentry_smg_animated_obj";
	level.sentry_settings[ type ].placementmodel 	 = "weapon_sentry_smg_obj";
	level.sentry_settings[ type ].placementmodelfail = "weapon_sentry_smg_obj_red";
	level.sentry_settings[ type ].health 			 = shielded_sentry_health;
	level.sentry_settings[ type ].no_steam			 = true;
	
	// turrent weapon override
	level.sentry_settings[ type ].damage_smoke_time = 0;
	level.sentry_settings[ type ].weaponInfo 	= "sentry_smg";
	level.sentry_settings[ type ].targetname 	= "sentry_smg";
}
...
sentry_init( team, sentryType, owner )
{
	...
	if ( isSP() )
	{
		if ( isdefined( level.sentry_settings ) 
		    && isdefined( level.sentry_settings[ "sentry_minigun" ] )
		    && isdefined( level.sentry_settings[ "sentry_minigun" ].bullet_armor ) )
		{
...
sentry_player_use_wait()
{
	...
	for ( ;; )
	{
		...
		if ( !isSP() )
		{
			...
			player PlayLocalSound( "sentry_pickup" );
		}
		else
		{
			player PlaySound( "sentry_pickup" );	
		}
