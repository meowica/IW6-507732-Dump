...
meleeStruggleVsAI()
{
	...
	{	// this guy is about to get meleed by someone else.  leave him alone.
		combatIdle();
		return;
	}
 
	level notify( "dog_attacks_ai", self, self.enemy );
	self.enemy notify( "dog_attacks_ai", self );
	...
	{
		if( IsDefined( level._effect[ "dog_bite" ] ) && IsDefined( level._effect[ "dog_bite" ][ meleeSeq] ) && IsDefined(  self.enemy ) )
		{
			PlayFXOnTag( level._effect[ "dog_bite" ][ meleeSeq], self.enemy, "TAG_EYE" );
		}
...
combatIdle()
{
	...
	idleAnims = [];
	idleAnims[ 0 ] = %iw6_dog_attackidle;
	idleAnims[ 1 ] = %iw6_dog_attackidle_bark;
