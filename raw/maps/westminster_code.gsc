...
aggressively_load_ai( vehicle_load_ai )
{
	...
	// IW6 Hack to fix guy not getting to position bug for loading into truck
	driver = undefined;
	foreach ( ai in vehicle_load_ai )
	{
		if ( ai == level.sas_leader )
		{
			// the other guy
			driver = vehicle_load_ai[ 1 ];
			break;
		}
	}
...
//	if ( IsDefined( driver ) )
//	{
//		dir = AnglesToRight( self.angles + ( 0, 180, 0 ) );
//		origin = self.origin + ( dir * 80 );
//		driver ForceTeleport( origin, ( 0, 0, 0 ) );
//	}
...
player_bail( guy )
{
	...
	thread set_audio_zone( "london_tube_end", 2 );
...
level_end( ent )
{
	...
	maps\_loadout_code::SavePlayerWeaponStatePersistent( "london", true );
	nextmission();
}
