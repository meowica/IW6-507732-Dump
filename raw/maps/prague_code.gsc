...
#include maps\prague_util;
#include maps\prague_util_stealth;
...
//	level.player_legs Hide();
	...
	thread spawn_resistance_guys_b();
...
spawn_resistance_guys_b()
{
	wait( 0.2 );
	level notify( "cleanup_dead_bodies" );
	cleanup_volume( "intro_cleanup_volume", true, false, true );
	wait( 0.1 );
	spawners = getentarray( "sewer_resistance_group_b" , "targetname" );
	thread idle_spawner_think( spawners );
}
...
sewer_price_think()
{
	...
	noself_delayCall( 2, ::PlayFXOnTag, getfx( "water_emerge" ), self, "J_Neck" );
...
alcove_scene_dialogue()
{
	...
	{
		if ( !level.Console && level.player is_player_gamepad_enabled() )
		{
			thread display_hint_timeout( "hint_prone_gamepad", 5 );
		}
		else
		{
...
//	array_spawn_targetname( "tunnel_kickers" );	
	...
	if ( Distance2d( level.player.origin, level.sandman.origin ) < 300 && (diff != "fu") )
	{
		if ( !level.Console && level.player is_player_gamepad_enabled() )
		{
			delayThread( 1.2, ::display_hint_timeout, "hint_prone_gamepad", 5 );
		}
		else
		{	
	...
	foreach ( org in origins )
	{
		if ( org.origin[2] > level.player.origin[2] )
		{
			if ( cointoss() )
			{
				...
				noself_delayCall( RandomFloatRange(0,1) , ::PlayFX, getfx( org.script_fxid ), org.origin );
				times += 1;
			}
		}
		if ( times > 5 )
...
cargo_heli_think()
{
	...
	if ( !flag( "start_sewers" ) )
		self thread maps\_vehicle::aircraft_wash( btr );		
...
//	self script_delay();
	...
	foreach ( n in node )
	{
		...
		noself_delayCall( num + RandomFloatRange( -1*num, 0.5 ), ::playfx, getfx( fxid ), n.origin );
	}
