...
{
	return int( tableLookup( "sp/allChallengesTable.csv", 0, refString, 9 + ((state-1)*2) ) );
}
...
{
	prestigeShopString = tableLookupByRow( "sp/allchallengesTable.csv", challengeIndex, CH_PRESTIGE_COL );
	...
	rankString = tableLookupByRow( "sp/allchallengesTable.csv", challengeIndex, CH_LEVEL_COL );
	if ( rankString != "" )
	{
		rank = self maps\_rank::getRank();
		if ( rank < int( rankString ) )
			return false;
	}
	
	challengeString = tableLookupByRow( "sp/allchallengesTable.csv", challengeIndex, CH_CHALLENGE_COL );
	if ( challengeString != "" )
	{
...
updateChallenges()
{
	...
	foreach ( challengeRef, challengeData in level.challengeInfo )
	{
		...
		assertEx( isDefined( challengeData["index"] ), "Challenge index not defined: " + challengeRef + " from " + level.challengeInfo.size + " total challenges" );
		challengeIndex = challengeData["index"];
		
		if ( !self isChallengeUnlocked( challengeRef, challengeIndex ) )
			continue;
