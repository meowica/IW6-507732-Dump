...
spawn_enemy_bike()
{
	...
	if ( distance( spawn_pos, level.player.origin ) < 140 )
	{
		return;
	}	
...
//			bike hide();
	...
	if ( isalive( bike ) && bike.wipeout && !flag( "race_complete" ) )
		wait( 5 );
...
update_position_on_spline()
{
	...
	for ( ;; )
	{
		...
		dist = distance( self.origin, last_org );
		progress += dist;
		last_org = self.origin;
...
match_player_speed( maxaccell, maxdecel )
{
	...
	else
	{
		...
		if ( progress < -100 )
		{
			if( !self ent_flag( "dialog_six" ) )
			{
				self ent_flag_set( "dialog_six" );
				self delaythread( 12, ::ent_flag_clear, "dialog_six" );
...
track_player_progress( org )
{
	...
	for ( ;; )
	{
		...
		if ( flag( "ai_snowmobiles_ram_player" ) )
		{
			...
			level.enemy_snowmobiles = array_removedead( level.enemy_snowmobiles );  // I got an error in Getclosest.. don't understand how this hasn't come up, but here's a defensive fix.
