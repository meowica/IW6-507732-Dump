...
system_state_spotted()
{
	while ( flag( "_stealth_spotted" ) )
	{
		...
		flag_waitopen_or_timeout( "_stealth_spotted", level._stealth.logic.detection_timeout * 1.25 );
	}
	
	// assume when you get here, no one is spotted anymore, so clear everyone's spotted flag	
	array = level._stealth.group.groups;
	foreach ( group_name, group in array )
	{
...
{
	ai = group_get_ai_in_group( group_name );
	array_thread( ai, ::notify_level_on_enemy, group_name );
	
	level endon( "enemy_" + group_name );
	
	if ( ai.size > 0 ) // only do this if there are enemies in this group.  Otherwise, it's definitely clear.
	{	
	...
	level notify( "enemy_" + group_name + "_stop" );
...
/* *
 * Had to add this so we can break out of -system_state_try_clear_flag- when AI acquire a new enemy
 * */

notify_level_on_enemy( group_name )
{
	notify_string = "enemy_" + group_name;
	self endon( "death" );
	level endon( notify_string );
	level endon( notify_string + "_stop" );
