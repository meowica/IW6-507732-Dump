...
{
	//""Blood Brothers""
	//"October 11th - 07:01:[{FAKE_INTRO_SECONDS:30}]"
	//"Yuri"
	//"Task Force 141 - Disavowed"
	//"Prague, Czech Republic"
	intro_screen_create( &"PRAGUE_INTROSCREEN_LINE_1", &"PRAGUE_INTROSCREEN_LINE_5", &"PRAGUE_INTROSCREEN_LINE_2" );
//	intro_screen_custom_func( ::prague_intro );
	...
	add_hint_string( "hint_prone_gamepad", &"PRAGUE_HINT_PRONE_GAMEPAD", ::should_break_prone_hint );
...
start_sewers()
{
	thread maps\_utility::set_audio_zone("prague_sewer"); // start prague_zodiac zone
...
sewers_setup()
{
	...
	maps\_treadfx::setvehiclefx( "script_vehicle_mi17_woodland_fly_cheap", "water", "fx/treadfx/heli_water" );
...
flag_set_trigger()
{
	maps\_trigger::trigger_flag_set( self );
}
...
prague_intro()
{
	...
	flag_wait( "fade_up" );
	thread maps\_introscreen::introscreen_generic_black_fade_in( 4, 5 );
	level.player delayCall( 4.0, ::FreezeControls, false );
	flag_wait( "city_reveal" );
	delayThread( 9.25, maps\_introscreen::introscreen_feed_lines );
}
