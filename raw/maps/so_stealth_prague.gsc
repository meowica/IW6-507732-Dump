...
open_doors()
{
	...
	door = maps\prague_util::link_door_to_clips( "red_house_entrance" );
	thread rotate_door( door, ( 0, 110, 0 ), 0.1 );
	
	// Last door before church uses link to organize pieces
	door = maps\prague_util::link_door_to_clips( "white_building_door" );
	...
	while ( 1 )
	{
		...
		if ( spawn_locs.size )
		{
			...
			foreach ( loc in spawn_locs )
			{
				...
				ai thread maps\prague_util::light_on_gun();
...
rain_think()
{
	...
	while ( 1 )
	{
		...
		if ( IsDefined( level.player2 ) && common_scripts\utility::distance_2d_squared( level.player.origin, level.player2.origin ) > 1450 * 1450  )
		{
...
encounter_church_spawn_street_patrol()
{
	...
	foreach ( ai_enemy in ai_enemies )
	{
		...
		ai_enemy thread maps\prague_util::light_on_gun();
...
on_spawn_ai_alley_enemy()
{
	...
	self thread maps\prague_util::light_on_gun();
...
on_spawn_ai_yard_stand()
{
	...
	if ( self.type != "dog" )
	{
		self maps\prague_util::light_on_gun();
	}
}
...
encounter_quad_execution_think()
{
	...
	enemy maps\prague_util::light_on_gun();
...
encounter_shop_spawn_ai()
{
	...
	array_spawn_function( spawners, maps\prague_util::light_on_gun );
	spawners_shop_enc = array_combine( spawners_shop_enc, spawners );
