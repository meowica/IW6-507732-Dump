...
failure_summary_display( skipDialog )
{
	if ( getdvarint( "so_nofail" ) )
		return;
	
	if ( !isdefined( skipDialog ) || !skipDialog )
		thread maps\_specialops::so_dialog_mission_failed_generic();
	
	MissionFailed();
	
	maps\_endmission_so::so_eog_summary_calculate( false );
	...
	maps\_endmission_so::so_eog_summary_display();
}
...
setup_leaderboard_data()
{
	...
	if( is_coop() && level.players.size == 2 )
		self setCommonPlayerData( "round", "xuidTeammate", get_other_player( self ) getXUID() );
	
	// ==== SpecOps timePlayed ==== setting total time played
	self setCommonPlayerData( "round", "timePlayed", self.so_eog_summary_data[ "time" ] );

	// ==== SpecOps kills ==== setting total kills individually
	self setCommonPlayerData( "round", "kills", self.so_eog_summary_data[ "kills" ] );
	...
	if ( is_survival() )
	{
		...
		self setCommonPlayerData( "round", "score", self.so_eog_summary_data[ "score" ] );
		
		// ==== survival wave ==== setting survival waves survived
		self setCommonPlayerData( "round", "wave", self.so_eog_summary_data[ "wave" ] );
		...
		self setCommonPlayerData( "round", "bulletsFired",allshotsfired );
		self setCommonPlayerData( "round", "bulletsHit", 	allshotshit );
		
		assert( isdefined( self.game_performance ) );

		assert( isdefined( self.game_performance[ "headshot" ] ) );
		self setCommonPlayerData( "round", "headshots", 	self.game_performance[ "headshot" ] );
		
		assert( isdefined( self.game_performance[ "revives" ] ) );
		self setCommonPlayerData( "round", "revives", 	self.game_performance[ "revives" ] );
		
		assert( isdefined( self.game_performance[ "credits" ] ) );
		self setCommonPlayerData( "round", "credits", 	self.game_performance[ "credits" ] );
		self setCommonPlayerData( "round", "totalXp", 	self.game_performance[ "credits" ] );
	}
	else
	{
		...
		self setCommonPlayerData( "round", "difficulty", stars );
...
specialops_mission_over_stats( was_success )
{
	...
	level.specOpsSettings maps\_endmission_so::setSoLevelCompleted( levelIndex );
	...
	foreach( player in level.players )
	{
		...
		if( setlevel != 0 )	
		{
			...
			level.specOpsSettings maps\_endmission_so::setSoLevelCompleted( max( 0, setlevel-1 ) );
		...
		if( upto != 0 )	
		{
			...
			{
				difficulty_string += "4";
				level.specOpsSettings maps\_endmission_so::setSoLevelCompleted( i );
			}
...
so_special_failure_hint()
{
	...
	foreach( player in level.players )
	{
		player thread maps\_player_death::player_throwgrenade_timer();
...
setDeadQuote_so()
{
	...
	deadquotes = common_scripts\utility::array_randomize( deadquotes );
	...
	if ( !maps\_utility::is_coop_online() )
	{
		...
		{
			if ( maps\_player_death::deadquote_recently_used( deadquotes[ i ] ) )
			{
				i++;
...
so_special_death_hint_tracker()
{
	...
	// Keep this above so_juggernaut_death as jugs can carry riotshields
	if ( so_riotshield_death( attacker, cause ) )
		return;
...
so_dog_death( attacker )
{
	...
	if ( isdefined( attacker.type ) && attacker.type == "alien" )
		return false;
...
so_riotshield_death( attacker, cause )
{
	if ( !IsDefined( attacker ) || !IsDefined( attacker.subclass ) || attacker.subclass != "riotshield" )
		return false;
	...
	if ( !is_survival() )
	{
		deadquotes[ deadquotes.size ] = "@DEADQUOTE_RIOTSHIELD_USE_EXPLOSIVE"; //"Try using Grenades, RPGs and Grenade Launchers against enemies with Riot Shields."
	}
		
	deadquotes[ deadquotes.size ] = "@DEADQUOTE_RIOTSHIELD_OUT_FLANK";
	deadquotes[ deadquotes.size ] = "@DEADQUOTE_RIOTSHIELD_DONT_CHARGE";
	
	so_special_failure_hint_set_array( deadquotes );
