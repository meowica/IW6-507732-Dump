...
enemy_event_reaction_flashbang( type )
{
	...
	self enemy_investigate_position( node, undefined, true );
}
...
enemy_investigate_position( node, position, play_react_anim )
{
	if ( isdefined( node ) )
	{
		wait randomfloat( 1 );
		
		if ( isdefined( play_react_anim ) && play_react_anim )
			self thread enemy_react_and_displace_to( node, position );
...
stealth_event_anim_defaults()
{
	...
	else
	{
		...
		array[ "explode" ]			 = ::enemy_animation_nothing;
	}
