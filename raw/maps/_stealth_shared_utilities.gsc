...
enemy_go_back()
{
	...
	{
		if ( self.type != "dog" )
			self stealth_set_run_anim( "_stealth_patrol_walk", true );
		else
		{
			self set_dog_walk_anim();
			self.script_growl = undefined;
...
enemy_react_and_displace_to( node, position )
{
	...
	anglesToPoint = VectorToAngles( origin - self.origin );
	...
	self stealth_anim_custom_animmode( self, "gravity", "_stealth_idle_to_run_" + direction );
}
...
enemy_runto_and_lookaround( node, position )
{
	...
	if ( self.type != "dog" )
		self stealth_set_idle_anim( "_stealth_look_around" );
}
...
enemy_announce_huh()
{
	...
	if ( flag( "_stealth_spotted" ) ) // if already spotted, don't say anything
		return;
...
enemy_announce_attack()
{
	...
	my_prefix = self get_country_prefix();
	
	self playsound( my_prefix + self.npcID + "_stealth_alert" );
}
...
enemy_announce_spotted( pos )
{
	...
	if ( self enemy_announce_snd( "spotted" ) )
	{
		...
		alias = my_prefix + self.npcID + "_stealth_alert";
		self playsound( alias );
	}

	if ( self enemy_announce_snd( "acknowledge" ) )
		self thread enemy_announce_spotted_acknowledge( self.origin, self get_country_prefix() );
}

enemy_announce_spotted_acknowledge( spotterPos, my_prefix )
{
	wait 1.5;	
	...
	alias = my_prefix + num + "_stealth_alert_r";
...
enemy_animation_custom( type )
{
	...
	if ( type != "doFlashBanged" )
	{
		...
		if ( isdefined( tag ) || isdefined( self.has_delta ) )
			node stealth_anim( self, anime, tag );
		else
			node stealth_anim_custom_animmode( self, "gravity", anime );
	}
...
stealth_set_run_anim( runanim, alwaysWalkForward )
{
	if ( isdefined( self.script_animation )
	    && isdefined( level.scr_anim[ "generic" ][ runanim + "_" + self.script_animation ] ) 
	   )
	{
		self set_generic_run_anim( runanim + "_" + self.script_animation, alwaysWalkForward );
		return;
	}
	
	self set_generic_run_anim( runanim, alwaysWalkForward );
}

stealth_set_idle_anim( idlenim )
{
	if ( isdefined( self.script_animation )
	    && isdefined( level.scr_anim[ "generic" ][ idlenim + "_" + self.script_animation ] ) 
	   )
	{
		self set_generic_idle_anim( idlenim + "_" + self.script_animation );
		return;
	}
	
	self set_generic_idle_anim( idlenim );
}

stealth_anim_custom_animmode( guy, custom_animmode, anime, tag )
{
	if ( isdefined( guy.script_animation )
	    && isdefined( level.scr_anim[ "generic" ][ anime + "_" + guy.script_animation ] ) 
	   )
	{
		self anim_custom_animmode_solo( guy, custom_animmode, anime + "_" + guy.script_animation, tag );
		return;
	}
	
	self anim_generic_custom_animmode( guy, custom_animmode, anime, tag );
}

stealth_anim( guy, anime, tag )
{
	if ( isdefined( guy.script_animation )
	    && isdefined( level.scr_anim[ "generic" ][ anime + "_" + guy.script_animation ] ) 
	   )
	{
		self anim_single_solo( guy, anime + "_" + guy.script_animation, tag );
		return;
	}
...
get_country_prefix()
{
	//todo: update spanish to whatever else gets added ( "spanish", etc);
	//self = AI
	voice = self.voice;	
	switch ( voice )
	{
