...
specialops_init()
{
	...
	set_mission_failed_override( ::so_mission_failed_override );
...
//	PreCacheShader( "hud_star69icon" );
	...
	// TODO: Redo hint system in iw6 thirdmode, in fact there should be a new info-bubble-system for all hint uses
	if ( !isdefined( level.so_campaign ) || level.so_campaign != "thirdmode" )
...
roundStat_init()
{
	...
	self setCommonPlayerData( "round", "kills", 0 );
	self setCommonPlayerData( "round", "killStreak", 0 );
	self setCommonPlayerData( "round", "deaths", 0 );
	self setCommonPlayerData( "round", "difficulty", 0 );
	self setCommonPlayerData( "round", "score", 0 );
	self setCommonPlayerData( "round", "timePlayed", 0 );
	self setCommonPlayerData( "round", "wave", 0 );
	self setCommonPlayerData( "round", "xuidTeammate", "0" );
	self setCommonPlayerData( "round", "totalXp", 0 );
	self setCommonPlayerData( "round", "scoreXp", 0 );
	self setCommonPlayerData( "round", "challengeXp", 0 );
}
...
=============
///ScriptDocBegin
"Name: so_SetModelFunc( <modelFunc> )"
...
=============
*/
so_SetModelFunc( modelFunc, starts_off )
{
	Assert( is_specialop() );
	if ( !IsDefined( starts_off ) )
		starts_off = false;
	thread maps\_loadout_so::UpdateModel( modelFunc );
}
...
fade_challenge_out( challenge_id, skipDialog )
{
	...
	maps\_endmission_so::so_eog_summary_calculate( true );
