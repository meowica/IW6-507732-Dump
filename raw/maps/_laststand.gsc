...
CONST_LASTSTAND_TYPE_OFF						= 0;		// When you gone, you stay gone
CONST_LASTSTAND_TYPE_HELPLESS					= 1;		// MW2 laststand where you get to use your gun briefly and then are helpless. In sp you die.
CONST_LASTSTAND_TYPE_GETUP						= 2;		// Players can pick themselves back up

CONST_LASTSTAND_DOWN_DELAY_AFTER_REVIVE			= 1.75;		// Time after getting up where the player is not allowed to be downed

CONST_LASTSTAND_GETUP_COUNT						= 9999;		// Max getups allowed
CONST_LASTSTAND_GETUP_INVULN_TIME				= 2.0;		// Time from the start of laststand to when the ai can actually hurt the player
CONST_LASTSTAND_GETUP_IGNORE_TIME				= 0.25;		// Time from the start of last stand that AI ignores the player
CONST_LASTSTAND_GETUP_BAD_PLACE_TIME			= 90.0;		// Time the bad place lives for before it is recreated if the player is still alive
CONST_LASTSTAND_GETUP_BAD_PLACE_RANGE			= 120;		// Range of the badplace created in laststand getup
CONST_LASTSTAND_GETUP_BAR_WIDTH					= 130;		// Width of the revive getup bar
CONST_LASTSTAND_GETUP_BAR_HEIGHT				= 12;		// Height of the revive getup bar
CONST_LASTSTAND_GETUP_ICON_SIZE					= 28;		// Width and height of the getup icon

CONST_LASTSTAND_GETUP_BAR_START					= 0.5;		// Fill amount of the getup bar the first time it is used
CONST_LASTSTAND_GETUP_BAR_START_PENALTY			= 0.0;		// Each time laststand is entered, the getup bar starting fill goes down by this amount
CONST_LASTSTAND_GETUP_BAR_START_MIN				= 0.2;		// Min starting fill amount of the getup bar 
CONST_LASTSTAND_GETUP_BAR_REGEN					= 0.0025;	// Percent of the bar filled for auto fill logic
CONST_LASTSTAND_GETUP_BAR_REGEN_FAST			= 0.01;		// Percent of the bar filled if the player is flagged for fast fill
CONST_LASTSTAND_GETUP_BAR_REGEN_TIME			= 0.05;		// Time between auto bar fills
CONST_LASTSTAND_GETUP_BAR_REGEN_NO_DMG_DELAY	= 3.0;		// If no damage has occured in this amount of time speed the bar regen up
CONST_LASTSTAND_GETUP_BAR_AI_KILL				= 1.0;		// Percent of the bar filled by an AI kill
CONST_LASTSTAND_GETUP_BAR_DAMAGE				= 0.1;		// Percent of the bar removed by AI damage
CONST_LASTSTAND_GETUP_BAR_DAMAGE_TIME			= 0.2;		// Min time in seconds between bar adjustments from AI damage
CONST_LASTSTAND_GETUP_BAR_CLEAN_DELAY			= 0.5;		// Time before the bar is removed, to allow the player to see the bar fill

CONST_LASTSTAND_GETUP_COUNT_START				= 0;		// The number of laststands in type getup that the player starts with
...
main()
{
	...
	// Function handler for systems that need to check whether killing a player
	// should put them down / in last stand
	level.laststand_kill_will_down_func = ::laststand_killing_will_down;
	...
	PreCacheShellShock( "laststand_getup_pc" );
...
laststand_on_load_finished()
{
	...
	level waittill( "load_finished_so" );
...
laststand_revive_ents_manager()
{
	...
	{	
		...
		// hide annoying triangle artifact under players' feet
		// PC only - end of dev cycle patching
		if ( !level.Console )
			revive_ent hide();
...
player_laststand_effect()
{
	...
	if ( level.console )
		...
		self player_shock_effect( "laststand_getup_pc", 60, true );
}
...
laststand_killing_will_down()
{
	return laststand_downing_will_fail() == false;
}
