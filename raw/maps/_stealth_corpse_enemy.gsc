...
enemy_corpse_saw_behavior()
{
	...
	if ( self.type != "dog" )
		self stealth_set_run_anim( "_stealth_combat_jog" );
	else
	{
		self clear_run_anim();
		self.script_growl = 1;
...
enemy_corpse_alert_level()
{
	...
	//if the enemy is allies (not player) we may just be doing ignoreme instead of the Stealth system on allies. Return.
	if(!isdefined( enemy._stealth ) && !isplayer( enemy ) )
		return;
