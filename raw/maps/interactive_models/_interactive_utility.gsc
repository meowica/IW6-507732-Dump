...
loop_anim( animArray, animName, ender, animRate )
{
	...
	while ( 1 )
	{
		self single_anim( animArray, animName, "loop_anim", false, animRate );
...
"Module: Anim"
"CallOn: An entity"
"MandatoryArg: <animArray>: An array.  Can contain animations, arrays of animations and weights (just like those found in level.scr_anim[animname])."
"MandatoryArg: <animName>: The animation index in the array, just like anime in anim_loop_solo."
"OptionalArg: <notifyStr>: String that the animation will notify for notetracks and when finished.  Defaults to single_anim."
"OptionalArg: <restartAnim>: Set to true to force the animation to start from the beginning, otherwise it will simply continue if it is already playing.  Defaults to false."
"OptionalArg: <animRate>: Playback speed for the animation.  Defaults to 1."
"Example: "
"SPMP: singleplayer"
///ScriptDocEnd
...
single_anim( animArray, animName, notifyStr, restartAnim, animRate )
{
	...
	if ( IsDefined( restartAnim) && restartAnim )
		self SetFlaggedAnimKnobRestart( notifyStr, animation, 1, 0.1, animRate );
	else
		self SetFlaggedAnimKnob( notifyStr, animation, 1, 0.1, animRate );
...
"Summary: Blends between animations in an array according to the speed parameter.  Keeps all animations playing in sync (without using loopsync in the animtree).  Note: uses SetAnimLimited, so be sure to set the parent node in the animtree separately."
"Module: Anim"
"CallOn: An entity"
...
blendAnimsBySpeed( speed, anims, animSpeeds, animLengths, blendTime )
{
	...
	if ( isSP() )	// We only blend in SP
	{
		...
		self setAnimLimited( anims[ i   ], 1 - fastWeight, blendTime, fastRate / speedRatio );
		self setAnimLimited( anims[ i+1 ],     fastWeight, blendTime, fastRate );
		for ( j=0; j<i; j++ )
		{
			speedRatio = animLengths[ i+1 ] / animLengths[ j ];
			self setAnimLimited( anims[ j ],     0.01, blendTime, fastRate / speedRatio );
		}
...
/*		if ( fastWeight > 0.5 )
		{
			self ScriptModelPlayAnim( anims[ i+1 ] );
		}
		else
		{
			self ScriptModelPlayAnim( anims[ i ] );
		}
*/	}
}
...
detect_events( notifyString )
{
	self endon( "death" );
	self endon( "damage" );
	self MakeEntitySentient( "neutral" );
...
// Draws a circle in 3D space
drawCircleUntilNotify( origin, radius, color, notifyEnt, notifyStr )
{
	numSegments = 16;
	for ( i=0; i<360; i+=(360/numSegments) )
	{
		j = i + (360/numSegments);
		thread maps\_utility::draw_line_until_notify ( origin + ( radius*Cos(i), radius*Sin(i), 0 ), origin + ( radius*Cos(j), radius*Sin(j), 0 ), color[0], color[1], color[2], notifyEnt, notifyStr );
	}
}
