...
pre_load()
{
	maps\_vehicle::build_deathfx_override( "script_vehicle_sas_van_physics", "sas_van_physics", "script_vehicle_sas_van_physics", "fx/explosions/large_vehicle_explosion_london", undefined, "car_explode" );
...
player_shadow()
{
	SetDvarIfUninitialized( "player_shadow", "1" );

	if ( GetDvarInt( "player_shadow" ) < 1 )
		return;

	shadow = Spawn( "script_model", level.player.origin );
	shadow character\character_sas_urban_smg::main();
	shadow Attach( getWeaponModel( "mp5_silencer" ), "tag_weapon_right" );
	
//	shadow DontInterpolate();
//	shadow DontCastShadows();
//	shadow Hide();
//	wait( 0.05 );
//	shadow CastShadows();
	shadow thread shadow_anim();
	
//	shadow LinkToPlayerView( level.player, "tag_origin", ( -20, 0, -60 ), ( 0, 0, 0 ), false );
	
	while ( 1 )
	{
		wait( 0.05 );
		offset = GetDvarInt( "player_offset", -20 );
		shadow.origin = level.player.origin + ( AnglesToForward( level.player.angles ) * offset );
		shadow.angles = level.player.angles;
	}
}
...
shadow_anim()
{
	idles = [ %casual_stand_v2_idle, %casual_stand_v2_twitch_radio, %casual_stand_v2_twitch_shift, %casual_stand_v2_twitch_talk ];
	run_forwards = [ %run_lowready_F ];
	crouches = [ %casual_crouch_idle ];
	reloads = [ %exposed_reload ];
	...
	while ( 1 )
	{
		...
		animation = undefined;
		if ( self.current_state == "run_forward" )
		{
		...
		else if ( self.current_state != "run_forward" && Length( level.player GetVelocity() ) > 10 )
		{
			self.current_state = "run_forward";
			old_player_origin = level.player.origin;
			animation = get_random_anim( run_forwards );
		}
		else if ( level.player IsReloading() )
		{
			if ( self.current_state != "reloading" )
			{
				self.current_state = "reloading";
				animation = get_random_anim( reloads );
			}
		}
		else
		{
			stance = level.player GetStance();
			if ( self.current_state != "stand" && stance == "stand" )
			{
				self.current_state = "stand";
				animation = get_random_anim( idles );
			}
			else if ( self.current_state != "crouch" && stance == "crouch" )
			{
				self.current_state = "crouch";
				animation = get_random_anim( crouches );
			}
		}
		
		if ( IsDefined( animation ) )
		{
			self thread shadow_playanim( animation );
		}
	}
}

shadow_playanim( animation )
{
	self notify( "stop_shadow_anim" );
	self endon( "stop_shadow_anim" );
	
	println( animation );
	
	self ClearAnim( %root, 0.2 );

	self setFlaggedAnimRestart( "shadow_anim", animation, 1, 0.2 );
	self waittillmatch( "shadow_anim", "end" );


	self.current_state = "none";
}

get_random_anim( animations )
{
	return animations[ RandomInt( animations.size ) ];
}
