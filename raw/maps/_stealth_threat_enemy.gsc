...
enemy_alert_level_warning1()
{
	...
	if ( isdefined( self.script_patroller ) )
	{
		if ( self.type != "dog" )
		{
			...
			if ( cointoss() )
				type = "b";
			self stealth_set_run_anim( "_stealth_patrol_search_" + type, true );
		}
		else
		{
...
enemy_alert_level_warning2()
{
	...
	if ( self.type != "dog" )
		self stealth_set_run_anim( "_stealth_patrol_cqb" );
	else
	{
		self clear_run_anim();
...
//	self.fixednode = true;
	...
	if ( self.type != "dog" )
	{
		...
		self stealth_set_run_anim( type, true );
	}
	else
	{
		self stealth_anim_custom_animmode( self, "gravity", "_stealth_dog_stop" );
		self set_dog_walk_anim();
	}
...
enemy_close_in_on_target()
{
	...
	while ( isdefined( self.enemy ) && self ent_flag( "_stealth_enabled" ) )
	{
		...
		// Handle stealth seek being turned off during the wait
		if ( isdefined( self.script_stealth_dontseek ) &&  self.script_stealth_dontseek == true )
			return;
	}
}
