...
main()
{
	...
	//AmbientPlay( "ambient_mp_lonestar" );
	
	maps\mp\_compass::setupMiniMap( "compass_map_mp_lonestar" );

	setdvar_cg_ng( "r_specularColorScale", 1.5, 25 );	
	setdvar_cg_ng( "r_diffuseColorScale", 1.375, 3.5 ); 
...
quakes()
{
	...
	add_quake_scriptable("qauke_script_telephone_wire", GetAnimLength( %mp_lonestar_earthquake_wire_loop ), true);
	...
	for(i=0;i<num_quakes;i++)
	{
		min_time = (1/num_quakes)*(i+.2);
		max_time = (1/num_quakes)*(i+.8);
...
quake_run_scriptables(scriptables)
{
	foreach(scriptable in scriptables)
	{
		...
		blend_time = 1;
		if(scriptable.quake_scriptable_time<=blend_time)
			blend_time = scriptable.quake_scriptable_time/2;
		scriptable delaycall(scriptable.quake_scriptable_time - blend_time, ::SetScriptablePartState, "main", "idle");
	}
}
...
quake_event_trigger_wait(func, var1, var2, var3, var4, var5)
{
	while(1)
	{
		...
		foreach(token in tokens)
		{
			switch ( token )
			{
				case "ground_collapse":
					self thread quake_event_trigger_wait( ::quake_event_move_to, ent,1,1,0,true);
					break;
				case "shake":
				...
				case "move_to_end":
					...
					self thread quake_event_trigger_wait( ::quake_event_move_to, ent, .5);
					break;
				case "gas_leak":
...
quake_event_move_to(quakeTime, ent, time, accel, decel, delete_at_end)
{
...
quake_hurt_trigger(hurt_trigger, damage_rate, damage)
{
	...
	while(1)
	{
		...
		RadiusDamage(player.origin, 10, damage, damage);
	}
}
...
{
	ent ScriptModelPlayAnim( level.quake_anims[anim_name] );
...
quake_event_gas_leak(quakeTime, ent)
{
	...
	while(1)
	{
		...
		ent waittill_notify_or_timeout("damage", 30);
...
quake_event_pole_fall_on_car(quakeTime, ent)
{
	...
	animated_prop ScriptModelPlayAnim( "mp_lonestar_police_car_crush_pole" );
