...
//	maps\mp\_water::waterShallowFx();
	...
	level.storm_stage = 0; // 0 is clear, clouds moving across sky. 1 is clouds set, light rain fx. 2 is clouds set, heavy rain fx and audio
	...
	AmbientPlay( "ambient_mp_fahrenheit" );
	
	maps\mp\_compass::setupMiniMap( "compass_map_mp_fahrenheit" );
	
  setdvar_cg_ng( "r_specularColorScale", 3, 6 );	
...
plant_anims()
{
	...
	level waittill("storm_stage_2");
	plant_set_stage_directional("stage_2", 180, 6, 1);
}
...
//	while ( true )
	{
		...
		{
			player VisionSetStage( current_stage, 3 );			
		}
...
sky_and_visionsets( match_duration, start_fraction, end_fraction )
{
	...
	level.skydome = GetEnt( "skydome", "targetname" );
	level.skydome thread rotate_skydome( start_fraction, end_fraction, level.stage_1_fraction * match_duration );
	...
	{		
		foreach ( player in level.players )
		{
			player VisionSetStage( level.storm_stage, 0.1 );
		}
	...
	if( start_fraction < level.stage_2_fraction )
	{
		...
		foreach ( player in level.players )
		{
			player VisionSetStage( level.storm_stage, transition_time );
		}
