...
//		RadiusDamage(self.origin, self.radius, self.max_damage, self.min_damage, attacker);
		foreach( participant in level.participants )
		{
			...
			{
				participant shellShock( "concussion_grenade_mp", stuntime );
			}
		}
	}
}
...
breach_use_watch()
{
	...
	self.useObject maps\mp\gametypes\_gameobjects::setUseTime( .25 );
	self.useObject maps\mp\gametypes\_gameobjects::setUseText( &"MP_BREACH" );
...
breach_onUse(player)
{
	...
	self breach_set_2dIcon("hud_grenadeicon_back_red");
	
	self.parent thread breach_warning_icon(self);
	
	sound_ent = self.parent get_breach_target("sound");
	if(IsDefined(sound_ent))
	{
		...
		sound_ent PlaySound("breach_explode");
	}
	
	delayThread(3, ::breach_set_2dIcon, undefined);
	self.parent notify("breach_activated",player);
}
