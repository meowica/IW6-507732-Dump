...
Jump( startPos, startAngles, endPos, endAngles, nextPos, jumpCBs )
{

	maps\mp\agents\alien\_alien_anim_utils::turnTowardsVector( endPos - startPos );
	oldTurnRate = self ScrAgentGetMaxTurnSpeed();
	self thread JumpInternal( startPos, startAngles, endPos, endAngles, nextPos, jumpCBs );
...
JumpInternal( startPos, startAngles, endPos, endAngles, nextPos, jumpCBs )
{
	self endon( "death" );
	...
	oldTurnRate = self ScrAgentGetMaxTurnSpeed();
	self thread jumpOrient( jumpInfo, endAngles, oldTurnRate );
	...
	self SetOrigin( endPos ); // Boo!
	
	// When SetAnimState() is called on the same frame as a SetOrigin(), the animation does not play
	// Ensure that the next animation will play :-(
	wait 0.05; 
...
jumpOrient( jumpInfo, endAngles, oldTurnRate )
{
	self endon( "death" );
	...
	{
		wait ( jumpInfo.jumpTime * start_orient_time );
	}
	...
	if ( DistanceSquared( self.angles, endAngles ) > threshold )
	{
		...
		turnRate = anglesDelta / ( jumpInfo.jumpTime * total_orient_time );
...
JumpPain( duration, endPos )
{
	...
	self waittill( "jump_pain", damageDirection, hitLocation, iDamage );
	...
	jump_pain_state = self maps\mp\agents\alien\_alien_anim_utils::getPainAnimState( "jump_pain", iDamage );
	jump_pain_index = self maps\mp\agents\alien\_alien_anim_utils::getPainAnimIndex( "jump", damageDirection, hitLocation );
	damage_degree = self maps\mp\agents\alien\_alien_anim_utils::getDamageDegree( iDamage );
