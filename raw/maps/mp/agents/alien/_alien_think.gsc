...
{
	if ( !self maps\mp\agents\_agents::agent_damage_finished( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, timeOffset ) )
		return false;
	...
	if ( belowCumulativePainThreshold() )
		return;

	notifyJumpPain( vDir, sHitLoc, iDamage );
	clearDamageHistory();
...
notifyJumpPain( damageDirection, hitLocation, iDamage )
{
	...
	self notify ( "jump_pain", damageDirection, hitLocation, iDamage );
}
...
belowCumulativePainThreshold()
{
	alien_type = get_alien_type();
	recentCumulativeDamage = getCumulativeDamage();
	
	return ( recentCumulativeDamage < level.alien_types[ alien_type ].attributes[ "min_cumulative_pain_threshold" ] );
}
...
ALIEN_JOG_CHANGE_DISTANCE = 1000;
...
alien_attack( enemy, attack_type )
{
	...
	switch ( attack_type )
	{
		...
		case "charge":
			...
			break;
		case "regen_health":
			maps\mp\agents\alien\_alien_elite::health_regen( enemy );
			break;
		case "explode":
...
handle_badpath( enemy )
{
	...
	{
		nodes = GetNodesInRadius( self.origin, 512, 128, 256, "node_pathnode" );
		if ( nodes.size > 0 )
		{
			self badpath_teleport( nodes[ RandomInt( nodes.size ) ] );
			self.badpathcount = 0;
		}
	}
	
	// If really far away, run to any node near player
	MAX_BADPATH_COMBAT_DISTANCE_SQR = 400.0 * 400.0;
	if ( DistanceSquared( self.origin, enemy.origin ) > MAX_BADPATH_COMBAT_DISTANCE_SQR )
	{
		//self thread restart_loop_near_enemy( enemy );
		nodes = GetNodesInradius( enemy.origin, 256.0, 0.0, 128 );
		if( nodes.size == 0 )
		{
			/#alien_path_error( "Unable to find any node within 256-unit to 800-unit radius.  Talk to level designer." );#/
			
			nodes = GetNodesInRadius( enemy.origin, 2000, 400, 256 );
		}
		if( nodes.size > 0 )
		{
			self ScrAgentSetGoalNode( nodes[ RandomInt( nodes.size ) ] );
	...
	// If we're already near player and are still in bad_path, path directly to a node very near player
	self close_range_retreat( enemy );
}

// Note JW: This is TEMP.  We should handle the badpath, not teleport the alien.
badpath_teleport( node )
{
...
{	
	...
	{
	case "spitter":
		return maps\mp\agents\alien\_alien_spitter::should_spitter_retreat();
	default:
		return true;
	}
}
...
get_retreat_node_rated( enemy, filters, nodes )
{
	...
	*/
	...
	while ( index < nodes.size )
	{
		...
		{
			if (( node.origin [ 2 ] - enemy.origin [ 2 ] ) > min_height 
			    && ( node.origin[2] - enemy.origin[2] ) < max_height )
			{
		...
		if ( use_cover_target_vector )
		{
			...
			compare_vector = VectorNormalize( enemy.origin - node.origin );
		}
		else
		{
			compare_vector = VectorNormalize ( node.origin - self.origin );
		...
		{
			dist = distance( node.origin, enemy.origin );
		...
		if ( min_dist_from_all_enemies_weight )
		{
			...
			foreach ( player in level.players )
			{
				if ( player != enemy )
				{
					if ( DistanceSquared( node.origin, player.origin ) < min_dist_from_all_enemies * min_dist_from_all_enemies )
					{
		...
		{
			if ( DistanceSquared( self.origin, node.origin ) > min_dist_from_current_position*min_dist_from_current_position )
			{
		...
		if ( enemy_los_weight > 0.0 && rating > best_node_rating - enemy_los_weight )
		{
			...
			trace_passed = SightTracePassed( enemy get_eye_position(), node.origin + ( 0, 0, 50 ), false, self );
			//num_traces++;
...
switchToJog()
{
	SLOWER_PLAYBACK_RATE = 1.0;
	
	self.movemode = "jog";
	self.previousplaybackrate = self.moveplaybackrate;
	self.moveplaybackrate *= SLOWER_PLAYBACK_RATE;
	self thread backToRunOnDamage();
}
...
backToRunOnDamage()
{
	...
	self.movemode = "run";
	self.moveplaybackrate = self.previousplaybackrate;
}
...
go_for_swipe( enemy, approach_node )
{
	...
	self run_near_enemy( melee_distance, enemy, approach_node );
	return "swipe";				
}
...
alien_melee( enemy )
{
	...
	{
		if ( self get_alien_type() != "spitter" )
		{
...
run_to_approach_node( approach_node, max_distance, enemy )
{
	...
	wait_till_reached_goal_or_distance_from_enemy( max_distance, enemy );
...
wait_till_distance_from_enemy( max_distance, enemy )
{
	...
	while ( true )
	{
		if ( DistanceSquared ( self.origin, enemy.origin ) < max_distance_sqr )
		{
			break;	
		}
		waitframe();
...
go_to_leaping_melee_position( enemy )
{
	...
	self ScrAgentSetGoalEntity( enemy, 250 );
...
{	
	...
	traceResult = TrajectoryCanAttemptAccurateJump( self.origin, AnglesToUp( self.angles ), enemy.origin, AnglesToUp( enemy.angles ), level.alien_jump_melee_gravity, 1.01 * level.alien_jump_melee_speed );
...
get_attack_num()
{
	...
	{
	case "spitter":
		return maps\mp\agents\alien\_alien_spitter::get_spitter_attack_num();
	default:
