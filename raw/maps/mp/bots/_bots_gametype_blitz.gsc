...
initialize_blitz_role()
{
	...
	attacker_limit = blitz_bot_attacker_limit();
	defender_limit = blitz_bot_defender_limit();
...
blitz_bot_attacker_limit()
{
	team_limit = bot_get_team_limit();
	return int( int(team_limit) / 2 ) + 1 + ( int(team_limit) % 2 );
}

blitz_bot_defender_limit()
{
	team_limit = bot_get_team_limit();
	return int( int(team_limit) / 2 ) - 1;
}
...
get_allied_attackers_for_team(team)
{
	...
	{
		if ( !IsAI(player) && player.team == team )
		{
