...
drone_play_looping_anim( droneAnim, rate )
{
	...
	{
		self [[self.drone_loop_override]]( droneanim,rate);
	}
	else
	{
...
drone_move()
{
	...
	for ( ;; )
	{
		...
		while ( lookaheadDistanceFromNode > nodes[ currentNode_LookAhead ][ "dist" ] )
		{
			...
			//-------------------------------------------------------------------------
			// LOGIC FOR STAIRS
			// Because of the way Drone movement is done, this is not perfect
			if( isdefined( do_stairs ) )
			{
				if( currentNode_LookAhead == 0 ) 
					assertmsg( "There must be a node between the drone spawner and stairs start node" );
				
				if( !isdefined( self.beforeStairAnim ) )
					self.beforeStairAnim = self.droneAnim; 
				
				stairsAnim = level.drone_anims[ self.team ][ "stairs" ][ do_stairs ];
				self drone_play_looping_anim( stairsAnim, self.moveplaybackrate );
				did_stairs = true;
			}
...
//					wait timeOfMove;
		...
		{
			sArray = strtok( self.cur_node[ "script_noteworthy" ], "_" );
			do_stairs = sArray[2];
		}
		else if( did_stairs == true )
		{
			if ( IsDefined( self.cur_node[ "script_noteworthy" ] ) && self.cur_node[ "script_noteworthy" ] == "stairs_end" )
			{	
				droneAnim = self.beforeStairAnim;
				self drone_play_looping_anim( droneAnim, self.moveplaybackrate );
				did_stairs = false; // did_stairs
				do_stairs = undefined;
			}
		}
	}
